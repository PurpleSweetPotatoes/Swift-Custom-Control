...
//        self.backgroundColor = UIColor("e3e7e7")
    ...
    private func createContentView() {
        ...
        if let title = self.title {
            let rect = title.boundingRect(with: CGSize(width: width, height: 100), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: [NSAttributedString.Key.font:self.titleFont], context: nil)
        ...
        let dic = [NSAttributedString.Key.font : self.infoFont,
                   NSAttributedString.Key.paragraphStyle: style]
        let infoRect = self.info.boundingRect(with: CGSize(width: max(maxWidth, width), height: 100), options: [.usesLineFragmentOrigin, .usesFontLeading, .truncatesLastVisibleLine], attributes: dic, context: nil)

        let contentLab = UILabel(frame: infoRect)
        contentLab.numberOfLines = 0
        contentLab.lineBreakMode = .byCharWrapping
        contentLab.font = self.infoFont
        contentLab.text = self.info
        contentLab.textColor = UIColor.white
        self.addSubview(contentLab)
        self.sizeW = max(maxWidth, contentLab.sizeW) + 40
